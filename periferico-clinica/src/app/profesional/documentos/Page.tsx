...
import type { CreateClinicalDocumentRequest, Diagnosis, FollowUpInstructions, SnomedCatalogItem} from "../../../types/clinical-document";
...
export default function NewClinicalDocumentPage() {
    ...
    const { reasons: consultationReasonsCatalog, loading: loadingConsultationReasons, error: errorConsultationReasons } = useConsultationReasons({
        autoFetch: true, // Cargar automáticamente al montar
    });

    // Hook para obtener estados de problema
    const { reasons: problemStatusCatalog, loading: loadingProblemStatus, error: errorProblemStatus } = useProblemStatus({
        autoFetch: true,
    });

    // Hook para obtener grados de certeza
    const { reasons: certaintyLevelsCatalog, loading: loadingCertaintyLevels, error: errorCertaintyLevels } = useCertaintyLevels({
        autoFetch: true,
    });
    ...
    useEffect(() => {
        if (problemStatusCatalog.length > 0 && certaintyLevelsCatalog.length > 0 && diagnoses.length === 1 && diagnoses[0]?.problemStatus === '') {
            setDiagnoses([{
                code: '',
                displayName: '',
                startDate: '',
    ...
    const availableConsultationReasons = useMemo(() => {
        return consultationReasonsCatalog.filter(
            catalogItem => !consultationReasons.some(
                selected => selected.code === catalogItem.code
            )
        );
    ...
    const validateDiagnosesComplete = useCallback((): boolean => {
        ...
        if (incompleteDiagnoses.length > 0) {
            console.log(`❌ Validación fallida: Hay ${incompleteDiagnoses.length} diagnóstico(s) incompleto(s). Complete todos los campos requeridos.`)
            return false;
        };
    ...
    const validateDiagnosisDates = useCallback((
        ...
        for (let i = 0; i < diagnoses.length; i++) {
            ...
            if (startDateVsCurrent < 0) {
                console.log(`❌ Validación fallida: La fecha de inicio del diagnóstico ${i + 1} (${diagnosis.startDate}) no puede ser menor a la fecha actual (${currentDate})`);
                return false;
            }
            ...
            if (startDateComparison > 0) {
                console.log(`❌ Validación fallida: La fecha de inicio del diagnóstico ${i + 1} (${diagnosis.startDate}) no puede ser mayor a la fecha de consulta (${consultationDate})`);
                return false;
            }
        }
    ...
    const validateNextConsultationDate = useCallback((
        ...
        const nextConsultationComparison = compareDates(
            followUpInstructions.nextConsultationDate,
            consultationDate
        );
        if (nextConsultationComparison <= 0) {
            console.log(`❌ Validación fallida: La fecha de próxima consulta (${followUpInstructions.nextConsultationDate}) no puede ser menor o igual a la fecha de consulta (${consultationDate})`);
            return false;
        }
    ...
    ): Array<Pick<SnomedCatalogItem, 'code' | 'displayName'>> => {
    ...
    const buildDocumentRequest = useCallback((
        ...
        validReasons: SnomedCatalogItem[]
    ...
    }, [patientId, mapConsultationReasonsToRequest, mapDiagnosesToRequest, mapFollowUpInstructionsToRequest]);
    // ===============================================================
    //          HANDLERS
    // ===============================================================
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            const documentRequest = buildDocumentRequest(
        ...
        <ProfessionalLayout>
            <div className="max-w-5xl mx-auto space-y-6 m-2">
                ...
                <p className="text-muted-foreground mt-2">
                    ...
                    <span className="ml-2 font-semibold text-[#2c3e50]">
                        ...
                        {diagnoses.map((diagnosis, index) => (
                            <div key={index.valueOf()} className="border border-gray-200 rounded-lg p-4 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <Label htmlFor={`diagnosisDisplayName-${index}`} className="mb-2">Descripción del diagnóstico *</Label>
                                        <Input
                ...
                <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <Label htmlFor="nextConsultation" className="mb-2">Próxima consulta(opcional)</Label>
                                <Input
                                    id="nextConsultation"
                            ...
                            <div>
                                <Label htmlFor="referral" className="mb-2">Referencia al alta(opcional)</Label>
                                <Input
                                    id="referral"
