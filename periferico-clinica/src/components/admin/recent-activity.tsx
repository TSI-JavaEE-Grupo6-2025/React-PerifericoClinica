...
export function RecentActivity() {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-[#2c3e50]">Actividad Reciente</CardTitle>
      </CardHeader>
      <CardContent>
              ...
              .map((activity, index) => (
                <div key={index} className="flex items-start gap-3">
                  <Avatar className="h-9 w-9">
                    <AvatarFallback className="bg-[#2980b9] text-white text-xs">
                      {activity.user
                        .split(" ")
                        .map((n) => n[0])
                        .join("")}
                  ...
                  <div className="flex-1 space-y-1">
                    <p className="text-sm">
                      <span className="font-medium text-[#2c3e50]">{activity.user}</span> {activity.action}
                    </p>
